/**
 * Full PDF lean type-tagger — all needs_review PDFs not yet tagged by haiku.
 * Scope: mime=application/pdf AND status=needs_review AND v2_model IS DISTINCT FROM 'claude-haiku-4-5'
 * Model: claude-haiku-4-5. source=pipeline_pdf_leantag_full.
 */
...
const PID_FILE = '/tmp/pdf-leantag-full.pid';
...
const SOURCE = 'pipeline_pdf_leantag_full';
const MIN_CONFIDENCE = 0.92;
const LOG_FILE = '/tmp/pdf-leantag-full.log';
const DRIVE_TIMEOUT_MS = 25000;
const CONCURRENCY = 10;
const BRIDGE_MSG_ID = '1781999120484-bridge-6joch';
...
async function main() {
  acquireLock();
  log(`=== PDF LEANTAG FULL — Haiku concurrency=${CONCURRENCY} | PID=${process.pid} ===`);
  ...
  // Count scope: needs_review PDFs not yet haiku-tagged
  const { count: totalScope } = await supabase
    .from('massivlust_unclassified_files')
    .select('id', { count: 'exact', head: true })
    .eq('status', 'needs_review')
    .eq('mime_type', 'application/pdf')
    .or('v2_model.is.null,v2_model.neq.claude-haiku-4-5');
  log(`Scope (needs_review PDF, not haiku): ${totalScope}`);
  ...
  while (true) {
    ...
    log(`[PROGRESS] ${stats.attempted}/${totalScope} | ${rate}/min | typed=${stats.typed} classified=${stats.classified} withProject=${stats.withProject} err=${stats.errors} $${stats.totalCost.toFixed(2)}`);

    // Progress report every ~1000
    if (stats.attempted > 0 && Math.floor(stats.attempted / 1000) > Math.floor((stats.attempted - files.length) / 1000)) {
      const topTypes = Object.entries(stats.docTypes).sort((a, b) => b[1] - a[1]).map(([k, v]) => `${k}(${v})`).join(', ');
      await sendBridge(`PDF LEANTAG FULL — FREMDRIFT @${stats.attempted}/${totalScope} (${SOURCE})

Rate: ${rate} PDF/min | $${stats.totalCost.toFixed(2)} totalt | $${(stats.totalCost / Math.max(stats.processed, 1)).toFixed(4)}/PDF
Dok-typer: ${topTypes}
Typet: ${stats.typed} | Klassifisert: ${stats.classified} | Med prosjekt: ${stats.withProject} | Feil: ${stats.errors}`);
  ...
  log(`\n=== FERDIG === rate=${rate}/min cost=$${stats.totalCost.toFixed(3)} per=$${(stats.totalCost / Math.max(stats.processed, 1)).toFixed(4)}`);
  ...
  await sendBridge(`PDF LEANTAG FULL FERDIG (${SOURCE})

Behandlet: ${stats.attempted}/${totalScope} | Rate: ${rate} PDF/min | ${elapsed.toFixed(1)} min
Dok-typer: ${topTypes}
Typet (needs_review + type): ${stats.typed} | Klassifisert (>=0.92 + prosjekt): ${stats.classified} | Personlig: ${stats.personal}
Med prosjekt-hint: ${stats.withProject} | Ingen resultat: ${stats.noResult} | Feil: ${stats.errors}
Kostnad: $${stats.totalCost.toFixed(3)} totalt | $${(stats.totalCost / Math.max(stats.processed, 1)).toFixed(4)}/PDF`);
}
