...
function classifyBatch(prompt) {
  const tmpFile = join(tmpdir(), `studio-clf-${Date.now()}.txt`);
...
async function main() {
  console.log(`=== STUDIO CLASSIFY LOOP — batch ${BATCH_SIZE}, claimer: ${CLAIMER} ===`);
  ...
  while (true) {
    const { data: claimed, error: claimErr } = await supabase.rpc('massivlust_claim_classification_batch', {
    ...
    for (let j = 0; j < claimed.length; j++) {
      ...
      stats[cat.method === 'auto_classified' ? 'auto_classified' : cat.method === 'suggested' ? 'suggested' : cat.method]++;
      saveRows.push({
        id: file.id,
        method: cat.method,
    ...
    const { data: saved, error: saveErr } = await supabase.rpc('massivlust_save_classifications', {
      p_rows: saveRows
    });
    if (saveErr) {
    ...
    if (totalProcessed > 0 && totalProcessed % 1000 < BATCH_SIZE) {
      console.log(`\n=== ${totalProcessed} CHECKPOINT ===`);
    }
  }

  const elapsed = Math.round((Date.now() - startTime) / 60000);
  console.log(`\n=== COMPLETED: ${totalProcessed} files in ${elapsed} min ===`);
  console.log(`Stats: ${JSON.stringify(stats)}`);

  await supabase.from('massivlust_sync_runs').insert({
    source: 'studio_classify_loop',
