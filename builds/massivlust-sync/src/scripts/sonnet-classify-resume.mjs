...
const CLAIMER = 'resume-jun9';
const MODEL = 'claude-sonnet-4-6';
const QUOTA_STOP_PCT = 90;
const BATCH_DELAY_MS = 90000; // 90 sec between batches (throttled per Max request)
...
function buildPrompt(files) {
  ...
  return `Classify ${files.length} files for Massiv Lust AS (massivtre/CLT timber construction company, Norway).
...
- business_admin items: confidence = 0.90
...
CONTACT → PROJECT HINTS:
- ZEROexpo, Zero Expo → Tvildemoen (5b925045)
- Tron Meyer → architect, likely Adventveien projects
- Carsten Hovind, carsten@massivlust.no → Massivlust internal
- Motek, julie@motek.no → supplier, business_admin unless project mentioned
- Backe, Backe Prosjekt → general contractor, check for specific project
- Elise Kristoffersen, elise@flo.as → Flo AS, Brygghus/Telemark bryggeri
- Stolp-Larsson → Adventveien 15

FOLDER PATH CONTEXT:
The "mappe:" field shows the Drive folder path. Use it as a strong signal:
- Project name in path → confidence 0.95
- "42 Programmer til CNC" or "CNC Programmer" → business_admin unless project identifiable
- Numeric-only folders ("01", "05", "12") = NOT useful

DISAMBIGUATION:
Breivikveien cluster: 14B (bca855da)=default, Hus B (28d307d0)=explicit "Hus B", tildekking (bfabfb3b), Tildekking Hus B (277a69fc), Brevik prosjekt (6cca870f)=separate!
...
Resaland: påbygg (4ce3cd7c) vs Regulering (aab3dcbc)
Ulsåk/Ullsåk → Ulsåk Barnehage (82312e64)

FILES:
${fileList}

Return ONLY a JSON array, one object per file in order:
[{"index":1,"project_id":"<exact uuid or null>","confidence":<0.0-1.0>,"is_personal":<true|false>,"category":"<project|business_admin|personal|unknown>","reason":"<15-30 words>"},...]`;
}
...
function classifyBatch(prompt) {
  const tmpFile = join(tmpdir(), `clf-resume-${Date.now()}.txt`);
  writeFileSync(tmpFile, prompt);
  try {
    const raw = execSync(
      `cat "${tmpFile}" | claude --print --model ${MODEL} --output-format json`,
      { timeout: 600000, maxBuffer: 4 * 1024 * 1024, encoding: 'utf-8' }
    ).trim();
    if (!raw) return null;
    let text = raw;
    try {
      ...
      if (envelope.is_error) {
        const msg = String(envelope.result || '').toLowerCase();
        if (msg.includes('quota') || msg.includes('rate limit') || msg.includes('usage limit')) {
          console.error(`  QUOTA LIMIT DETECTED — stopping`);
          quotaExceeded = true;
        }
        return null;
      }
...
function categorize(result) {
  ...
  if (projectId && !PROJECT_IDS_SET.has(projectId)) {
    console.warn(`  [UUID-REJECT] ${projectId}`);
    projectId = null;
  }
...
async function getRemainingCount() {
  const { count } = await supabase
    ...
    .or('v2_processed_at.is.null,v2_model.neq.sonnet-4-6');
  return count || 0;
}

async function main() {
  console.log(`=== SONNET CLASSIFY RESUME (needs_review) — batch ${BATCH_SIZE}, claimer: ${CLAIMER} ===`);
  console.log(`Auto-stop at >${QUOTA_STOP_PCT}% quota. Filter: status=needs_review AND (v2_processed_at IS NULL OR v2_model != sonnet-4-6)`);
  ...
  while (true) {
    if (quotaExceeded) {
      console.log(`\nQuota limit reached — stopping at ${totalProcessed} files.`);
      break;
    }

    const { data: claimed, error: claimErr } = await supabase.rpc('massivlust_claim_needs_review_batch', {
      p_size: BATCH_SIZE, p_claimer: CLAIMER
    });
    if (claimErr) { console.error(`Claim error: ${claimErr.message}`); await delay(10000); continue; }
    if (!claimed?.length) { console.log('No more files to process.'); break; }

    console.log(`\nBatch: ${claimed.length} files claimed`);
    const prompt = buildPrompt(claimed);
    let results = classifyBatch(prompt);

    if (quotaExceeded) {
      for (const f of claimed) {
        await supabase.from('massivlust_unclassified_files').update({ v2_method: null }).eq('id', f.id);
      }
      break;
    }

    if (!results || results.length !== claimed.length) {
      console.warn(`  Retry: got ${results?.length ?? 'null'}, expected ${claimed.length}`);
      await delay(5000);
      results = classifyBatch(prompt);
    }

    if (!results || results.length !== claimed.length) {
      console.error(`  Batch failed — releasing claims`);
      for (const f of claimed) {
        await supabase.from('massivlust_unclassified_files').update({ v2_method: null }).eq('id', f.id);
      }
      totalErrors += claimed.length;
      await delay(10000);
      continue;
    }
    ...
    for (let j = 0; j < claimed.length; j++) {
      ...
      const statKey = cat.method === 'auto_classified' ? 'auto_classified' : cat.method === 'suggested' ? 'suggested' : cat.method;
      if (statKey in stats) stats[statKey]++;
      saveRows.push({
        id: file.id,
        method: cat.method,
        confidence: cat.confidence,
        project_id: cat.projectId || null,
        is_personal: cat.isPersonal,
    ...
    console.log(`  Done: ${totalProcessed} total | Remaining: ~${remaining} | ${elapsed}min elapsed | auto:${stats.auto_classified} sug:${stats.suggested} admin:${stats.business_admin} pers:${stats.personal} unk:${stats.unknown} err:${totalErrors}`);
  ...
  const elapsed = Math.round((Date.now() - startTime) / 60000);
  const status = quotaExceeded ? 'stopped_quota' : totalErrors === 0 ? 'success' : 'partial';
  console.log(`\n=== ${status.toUpperCase()}: ${totalProcessed} files in ${elapsed} min ===`);
  console.log(`Stats: ${JSON.stringify(stats)}`);

  if (totalProcessed > 0) {
    await supabase.from('massivlust_sync_runs').insert({
      source: 'sonnet_classify_batch_resume_jun9',
      status,
      started_at: new Date(startTime).toISOString(),
      ended_at: new Date().toISOString(),
      rows_in: totalProcessed + totalErrors,
      rows_upserted: totalProcessed,
